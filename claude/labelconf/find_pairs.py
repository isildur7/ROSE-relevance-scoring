...
"""Rank cross-label look-alike pairs by cosine distance and show them.

For each label-0 patch we find its nearest label-1 patches in UNI2 embedding
space, then surface the globally closest (label-0, label-1) pairs -- patches
that look alike but are annotated differently. Each label-1 (relevant) patch is
shown at most once; a label-0 patch may recur.

Outputs (under out_dir): pairs.csv, pairs.png.
"""
...
def main(
    ...
    """Rank cross-label look-alike pairs and write CSV + a montage.

    Args:
        embeddings_npz: Cached UNI2 embeddings from embed_patches.py.
        annotations_parquet: Source of top-left pixel coordinates.
        patch_dir: Directory holding the patch JPEGs (for the montage).
        out_dir: Output directory for pairs.csv / pairs.png.
        n_pairs: Pairs to surface.
    ...
    pairs = select_pairs(candidates, n=n_pairs)

    rows = _rows_for_csv(pairs, coord)
    ...
    _montage(
        ...
        Path(patch_dir),
        coord,
        "Closest cross-label look-alike pairs (label 0 vs label 1)",
        out_dir / "pairs.png",
    )
    logger.info("Wrote montage to %s", out_dir / "pairs.png")
