...
def main(
    ...
    log.info("Test split: %d patches (%d positive, %d negative)",
             len(test_df), test_df["label"].sum(), (test_df["label"] == 0).sum())

    nw = num_workers
    loader_kwargs: dict = dict(num_workers=nw, pin_memory=True, persistent_workers=nw > 0)
    if nw > 0:
    ...
    print(classification_report(labels, preds, target_names=["Non-informative", "Informative"]))
